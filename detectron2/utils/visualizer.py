...
#output difrectory = dir_prefix + "_Output"/"_OutputSemantic"
#要对其他方法做分割的话，只要修改这个dir_prefix的字符串内容就可以了。
#dir_prefix = "medical_50_MRI";
...
class Visualizer:
    ...
    def draw_panoptic_seg_predictions(
        self, panoptic_seg, segments_info,currentProcessing,dir_prefix, area_threshold=None, alpha=0.7
    ):
        """
        ...
        for mask in masks:
            if os.path.exists(dir_prefix+"_Output") is False:
                os.mkdir(dir_prefix+"_Output")             
            if os.path.exists(dir_prefix+"_Output/"+str(currentProcessing)) is False:
                os.mkdir(dir_prefix+"_Output/"+str(currentProcessing))
            cv2.imwrite(dir_prefix+"_Output/"+str(currentProcessing)+"/maskInstance"+str(curInstance)+".png",mask.astype(np.float32)*255);
            #print(dir_prefix+"_Output/"+str(currentProcessing)+"/maskInstance"+str(curInstance)+".png");
            curInstance += 1;
