...
class Visualizer:
    ...
    def draw_panoptic_seg_predictions(
        self, panoptic_seg, segments_info,currentProcessing, area_threshold=None, alpha=0.7
    ):
        """
        ...
        for mask, sinfo in pred.semantic_masks():
            semanticIndex += 1;        
            if os.path.exists("RoadSceneCBFOutputSemantic/"+str(currentProcessing)) is False:
                os.mkdir("RoadSceneCBFOutputSemantic/"+str(currentProcessing))        
            #各个组成部分不用输出，只要完整的输出就好
            #cv2.imwrite("RoadSceneCBFOutputSemantic/"+str(currentProcessing)+"/semantic"+str(semanticIndex)+".png",mask.astype(np.float32)*255);
            print("RoadSceneCBFOutputSemantic/"+str(currentProcessing)+"/semantic"+str(semanticIndex)+".png");
        ...
        curInstance = 1
        #枚举每一个实例的分割结果0/1mask，将其输出到对应文件夹
        for mask in masks:
            if os.path.exists("RoadSceneCBFOutput/"+str(currentProcessing)) is False:
                os.mkdir("RoadSceneCBFOutput/"+str(currentProcessing))
            #cv2.imwrite("RoadSceneCBFOutput/"+str(currentProcessing)+"/maskInstance"+str(curInstance)+".png",mask.astype(np.float32)*255);
            print("RoadSceneCBFOutput/"+str(currentProcessing)+"/maskInstance"+str(curInstance)+".png");
            curInstance += 1;
    ...
    def draw_binary_mask(
        ...
        if area_threshold is None:
            area_threshold = 4096
        #cv2.imwrite("outputs/mask"+str(text)+".png",binary_mask.astype(np.float32)*255);
